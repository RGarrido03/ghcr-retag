...
RC=$(curl -sL -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.oci.image.index.v1+json"  "https://ghcr.io/v2/$INPUT_GHCR_REPO/manifests/$INPUT_OLD_TAG" -o $TMPFILE -w "%{http_code}")
if [ "$RC" -eq 200 ]; then
...
RC=$(curl -sL -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/vnd.oci.image.index.v1+json"  "https://ghcr.io/v2/$INPUT_GHCR_REPO/manifests/$INPUT_NEW_TAG" -X PUT -d "@$TMPFILE" -o $TMPFILE2 -w "%{http_code}")
if [ "$RC" -eq 201 ]; then
